...
class WorkerNode():
    """
    Class Containing All Functions and Data a worker is responsible for
    """
    ...
    def construct_model(model_received : List[Tuple[nn.Module, dict]]) -> nn.ModuleList:
        """
        Initializes each piece of the model and returns it in a ModuleList
        """
        model = []
    ...
    def construct_optimizer(self, optimizer_recieved : Tuple[optim.Optimizer, dict]):
        """
        Initializes the optimizer with the kwargs specified and collects all the 
        parameters of the model to pass to the optimizer
        """
        assert self.MODEL!=None, "Model must be sent first"
        optim,kwargs = optimizer_recieved
        params = []
        for module in self.MODEL:
            params.extend([param for param in module.parameters()])
    ...
    def handle_server_request(self, data: dict):
        ...
        if 'GET_CUDA_MEM' in data:
            ...
            self.OPTIMIZER = self.construct_optimizer(data["OPTIMIZER"])
